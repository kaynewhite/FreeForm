...
- `server/world.js` — Admin-only **world-state API** at `/api/world`. Per-shard sparse tile storage on disk at `data/world/<shard>.json`. Endpoints: `GET /:shard` (read full shard), `POST /:shard/paint` (batch paint/erase, max 1024 tiles per call), `POST /:shard/clear-layer`, `GET /_/tilesets` (parsed TSX summaries + image URLs for the editor's palette). Tile references are `"<tilesetName>:<localTileId>"` strings; every paint validates the tileset exists in `data/maps/` and the id is in range against a small in-memory TSX cache invalidated by mtime. Coords capped at ±2²⁰. Default layers `ground` + `decor` are auto-created. Shard JSON shape: `{ shard, tileSize, createdAt, updatedAt, layers: [{ name, tiles: { "x,y": "tileset:id" } }] }`.
- `client/world.html` / `world.js` / `world.css` — Admin-only **World Loom** at `/world.html`. Loads shard `default`, renders all layers (active layer fully opaque, others dimmed) onto a pannable canvas (shift+drag or middle-mouse to pan, scroll-wheel zoom via the slider 1×–6×). Tile palette below the canvas reuses `GET /api/world/_/tilesets`: each tileset rendered with a clickable grid; the chosen tile shows a gold selection box. Left-click a world tile = paint with the selected ref; right-click = erase. Each click does an optimistic in-memory write, fires `POST /api/world/default/paint`, and rolls back the local change if the server rejects (e.g. tileset deleted underneath). A small gold cross marks the world origin (0, 0).
