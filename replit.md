...
- **World painting is in-game, live.** There is intentionally no `/world.html` admin page — actual world editing happens inside the running game via `/command we`, `/command world_edit`, and `/command server_edit` (built alongside the PixiJS client). The in-game palette must stay screen-contained: only the palette panel scrolls, never the whole page.
