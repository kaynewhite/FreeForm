...
- `client/sprites.html` / `sprites.js` / `sprites.css` — Admin-only sandbox at `/sprites.html`. Renders 4 direction previews + a raw-sheet cropper. Per-card slice controls (N/W/H/X/Y + reset). The raw sheet supports **drag-to-crop**: drag a box around the first frame and the active card's slice updates live, ghost-previewing the N repeats stepping right. Zoom slider (1×–8×) for accurate cropping on tiny sheets. "Apply slice to all directions" copies the active card's W/H/X (and Y for per-direction sets) to the others. Linked from the account strip when role=admin.
